public class Pawn extends ChessPiece {
    ...
    public boolean canMoveToPosition(ChessBoard chessBoard, int line, int column, int toLine, int toColumn) {
        if (checkPos(line) && checkPos(column) && checkPos(toLine) && checkPos(toColumn) && chessBoard.board[line][column] != null) { // проверка, что фигура не выходит за пределы доски
            if (column == toColumn) {
                int dir;
                int start;

                if (color.equals("White")) {  // ход белой фигуры
                    dir = 1;
                    start = 1;
                } else { // ход черной фигуры
                    dir = -1;
                    start = 6;
                }
